...
\caption{Pseudocode for \ffttwo \rule[1.75ex]{0pt}{0pt}}
\begin{tabular*}{\textwidth}{ll}
{\sc Purpose:} & Computes the $m$ most significant digits
of probabilities \\
& $p_{rn+s}=p(r,s)=\bfZ{r,s}{1,n}/\bfZ{}{}$ \rule[-1.5ex]{0pt}{0pt} \\
{\sc Input:} & RNA sequence $\seq = \seqN$, distinct secondary
structures \strA, \strB of \seq, \\
& integer $m$ \rule[-1.5ex]{0pt}{0pt} \\
{\sc Output:} & Probabilities $p_{rn+s}=p(r,s)=\bfZ{r,s}{1,n}/\bfZ{}{}$ to $m$ significant digits for \\
& $r,s=0,\dots,n-1$ \rule[-1ex]{0pt}{0pt} \\
\hline
\end{tabular*}
\begin{algorithmic}[1]
...
\EndFor
\For{$k \gets 0, n^2-1$}
\State $p_k \gets \frac{\sum_{j=0}^{n^2-1} a_j \omega^{-kj}}{n}$
\Comment{Where $a_j$ comes from equation (\ref{xxx})}
...
\Comment{Return all $p_k$ for $0 \leq k < n^2-1$,
from equation (\ref{xxx})}
\EndFunction
\rule[-0.35ex]{0pt}{0pt}
\end{algorithmic}
\end{algorithm}
...
(Note that the software actually usesbase $2$ precision parameter,
with maximum of $27$, where $2^{27} \approx 10^8$) It is well-known that
...
% 17. //compute coefficients of $\frac{Y(x)}{\bfZ{}{}}$ using (\ref{eq:ajdef})
% 18. if $\dBP{\strA,\strB}$ even then
% 19.   for $k=0$ to $M-1$
% 20.     $r=\pi_1(k)$, $s=\pi_2(k)$
% 21.     if $k$ even
% 22.       $\frac{\bfZ{r,s}{1,n}}{\bfZ{}{}} = a_{k/2}$ from (\ref{eq:ajdef})
% 23.     else// $k$ odd
...
% 30.     else// $k$ odd
% 31.       $\frac{\bfZ{r,n}{1,n}}{\bfZ{}{}} = a_{(k-1)/2}$ from (\ref{eq:ajdef})
% 32.  for $k=0$ to $(N+1)^2$
