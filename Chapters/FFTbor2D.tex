...
\EndFor
\For{$k \gets 0, n^2-1$}
\Comment{Compute IDFT from equation (\ref{eq:ajdef})}
\State $a_k \frac{1}{n} \sum_{j=0}^{n^2-1} a_j\, \omega^{-kj}$
...
\EndFor
\For{$k \gets 0, M-1$}
\Comment{Change the polynomial back to degree $M$}
\If{$\dBP{\strA}{\strB} \bmod 2 = 0$}
\If{$k \bmod 2 = 0$}
\State $p_{\pi_1(k) \cdot n + \pi_2(k)} \gets a_{k/2}$
\Else
\State $p_{\pi_1(k) \cdot n + \pi_2(k)} \gets 0$
\EndIf
\Else
\If{$k \bmod 2 = 0$}
\State $p_{\pi_1(k) \cdot n + \pi_2(k)} \gets 0$
\Else
\State $p_{\pi_1(k) \cdot n + \pi_2(k)} \gets a_{\frac{k-1}{2}}$
\EndIf
\EndIf
\EndFor
\For{$k \gets 0, (N + 1)^2 - 1$}
