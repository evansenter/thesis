...
\ffttwo, like \fftbor described in \Chref{ch:fftbor}, is an algorithm
...
applications of \ffttwo until \Chref{ch:hermes}, where the software is
...
(same as in \Chref{ch:fftbor}), where
...
\eqnref{eq:ffttwo:sumBoltzFactors}. In the context of the Nussinov model,
...
(see \eqnref{eq:fftbor:validBP}), and otherwise $E_0(k,j)=0$, and
$\alpha(k) =
...
(\eqnref{eq:fftbor:lagrangeInterpolation})
...
(\Chref{ch:fftbor}), we will perform polynomial interpolation
by using Vandermonde matrices and the \fft (FFT).

The following theorem shows that a
recursion, analogous to \eqnref{eq:ffttwo:bfZabij},
...
\eqnref{eq:fftbor:fftborNussPolyAlpha}---however notationally we will use the
...
\fullZx in \eqnref{eq:ffttwo:zOfX} by
...
\EndFor
\For{$k \gets 0, n^2-1$}
\Comment{Compute IDFT from \eqnref{eq:ffttwo:aFromY}}
...
\eqnref{eq:ffttwo:zOfX}, then for any \nRoU
...
Let \fullZx be defined by \eqnref{eq:ffttwo:zOfX}, and
...
\eqnref{eq:ffttwo:zOfXEvenN} are zero.
\smallskip
...
use the previous recursions from \eqnref{eq:ffttwo:polyRecNussJac}
...
the previous recursions from \eqnref{eq:ffttwo:polyRecNussJac}
...
As described in \Chref{ch:fftbor}, the \fft is the $O(n \log n)$
...
Since we defined \bfY in \eqnref{eq:ffttwo:yColumn} by $\bfY =
...
\eqnref{eq:ffttwo:zOfX} can be computed using the FFT.
...
\Comment{From \eqnref{eq:ffttwo:trickForRoU1}}
\If{$k \leq \frac{M_0}{2}$}
...
\EndIf
\Else
\Comment{From \eqnref{eq:ffttwo:trickForRoU2}}
\If{$k \leq \frac{M_0}{2}$}
\State $y_k \gets \nu^{-k} \cdot \emZof{}{\nu^k}$
\Else
\State $y_k \gets -\nu^{-k} \cdot \overline{\emZof{}{\nu^{M_0-k}}}$
