...
\bfZ{k}{i,j} = 0,\, \mbox{for $k > 0$, $i \le j \leq i+\theta$},
\end{align}
...
\bfZ{k}{i,j} = \bfZ{k-b_0}{i,j-1} \hspace{5pt} +
\sum_{\substack{(s_r,s_j) \in \mathbb{B}, \\ i \leq r < j}} \hspace{5pt}
\sum_{w+w'=k-b(r)}
...
structure $T_{[i,j]}$ on $s_i,\ldots,s_j$ that is a \kNbr of
$\strSt_{[i,j]}$,
...
\bfZ{k}{1,n} &= \sum_{\substack{\str \text{ such that }\\ \dBP{\str,\strSt}=k}}
e^{\frac{-E_0(S)}{RT}}
\end{align}
...
\bfZ{k}{i,j} &= \bfZ{k-b_0}{i,j-1} + \sum_{\substack{s_r s_j \in \mathbb{B}, \\
i \le r<j}}\left(e^{\frac{-E_0(r,j)}{RT}}\sum_{w+w'=k-b(r)}
...
\[ c_k(i,j)= \bfZ{k}{i,j} =
\sum_{\substack{\str \text{ such that }\\ \dBP{\str,\strSt_{[i,j]}}=k}}
e^{\frac{-E_0(S)}{RT}}.
\]
...
\noindent {\sc Theorem 1:} Let $s_1,\ldots,s_n$ be a given RNA sequence.
...
\begin{align}
\label{eq:fftbor:rnaborNussPoly}
...
\noindent {\sc Proof:} First, some notation is necessary.
...
\begin{align}
\label{eq:fftbor:rnaborNussPolyForAlpha}
...
where
$y_0=\emZof{}{\alpha_0},\ldots, y_{n-1}=\emZof{}{\alpha_{n-1}})$ is thus quartic time \On{4} and quadratic space \On{2}.
...
\begin{align}
\label{eq:fftbor:alphaIsRootUnity}
...
Define \bfY by $\bfY = (y_1,\ldots,y_n)^{\text T}$, where
...
\noindent {\sc Algorithm} for \fftbor \hfill \break
...
% 6.  compute $P = (p_0,\ldots,p_{n-1})^{\text T}$ where $p_j =\frac{\sum_{k=0}^{n-1} a_k \omega^{-kj}}{n}$ by using FFT in (\ref{eqn:ajdef})
% 7.  for $k=0$ to $n-1$
...
that for all values $k=0,\ldots,n-1$, if \aPbi is a complex $n$th
