...
$p(x)$ is defined in \eqnref{eq:fftbor:pOfX}. This
...
\eqnref{eq:fftbor:pOfX}. By leveraging
...
\eqnref{eq:fftbor:maxDist} and the fact that the minimum number of
...
interpolating the polynomial $p(x)$, defined in \eqnref{eq:fftbor:pOfX},
rather than the polynomial \fullZx, defined in \eqnref{eq:fftbor:zOfX},
...
shown in \eqnref{eq:fftbor:lagrangeInterpolation} or by Gaussian elimination. Fewer
...
Taylor expansion of $F$]. For instance, in \eqnref{eq:fftbor:pOfX},
$[x^k]p(x) = \pk$, and in \eqnref{eq:fftbor:zOfX},
$[x^k]\fullZx = c_k(i,j)$.
...
Since we defined \bfY in \eqnref{eq:fftbor:yColumn} by $\bfY =
...
\eqnref{eq:fftbor:zOfX} can be computed, at least in principle,
...
\fullZ is the partition function defined in \eqnref{eq:fftbor:partFunc}.
Using the \fft to compute the \idft, it follows from
\eqnref{eq:fftbor:aFromY} that we can compute the probabilities $p_0,\dots,p_{n-1}$
that are coefficients of the polynomial
$p(x) = p_0 + p_1 x + \dots + p_{n-1}x^{n-1}$
defined in \eqnref{eq:fftbor:pOfX}. For genomics applications, we are
...
\EndFor
\For{$k \gets 0, n-1$}
\Comment{Compute IDFT from \eqnref{eq:fftbor:aFromY}}
...
\eqnref{eq:fftbor:zOfX}, for $n/2$ of the \nRoUs.
...
\eqnref{eq:fftbor:zOfX}, then for any \nRoU
