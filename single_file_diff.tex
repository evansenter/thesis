...
%DIF DEL single_file_beta.tex    Sat Aug  8 03:17:49 2015
%DIF ADD single_file_edits.tex   Sat Aug  8 03:18:09 2015
...
%DIFDELCMD < \item[\tiny{$\blacksquare$}] This work was done wholly or mainly while in candidature for a research degree at this University.
%DIFDELCMD < \item[\tiny{$\blacksquare$}] Where any part of this thesis has previously been submitted for a degree or any other qualification at this University or any other institution, this has been clearly stated.
%DIFDELCMD < \item[\tiny{$\blacksquare$}] Where I have consulted the published work of others, this is always clearly attributed.
%DIFDELCMD < \item[\tiny{$\blacksquare$}] Where I have quoted from the work of others, the source is always given. With the exception of such quotations, this thesis is entirely my own work.
%DIFDELCMD < \item[\tiny{$\blacksquare$}] I have acknowledged all main sources of help.
%DIFDELCMD < \item[\tiny{$\blacksquare$}] Where the thesis is based on work done by myself jointly with others, I have made clear exactly what was done by others and what I have contributed myself.\\
...
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Declaration{
...
%DIFDELCMD < Between the excellent introduction to this field of research and all of the
%DIFDELCMD < guidance and support he provided over my graduate school tenure, I could not have
%DIFDELCMD < asked for a better advisor.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \acknowledgements{\addtocontents{toc}{\vspace{1em}} % Add a gap in the Contents, for aesthetics
...
developed an algorithm and accompanying software for the \DIFdelbegin \DIFdel{minimum free energy
}\DIFdelend \DIFaddbegin \mfe
\DIFaddend formulation of the problem \citep{zuker:1981tf,zuker:1989im}. In 1990, John
...
further improving the accuracy of \DIFdelbegin \DIFdel{computational-based }\DIFdelend \DIFaddbegin \DIFadd{computational }\DIFaddend models.
...
The work of this thesis is based \DIFdelbegin \DIFdel{off }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the following four journal articles, alongside
...
whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend to an input reference structure \str is $k$.
...
probability $p_{x,y}$
of those structures whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend from \strS [resp. \strTresp] is $x$
[resp. $y$]. This program---\ffttwo---allows for the efficient approximation of
kinetic characteristics of RNA molecules\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend presented in \Chref{hermes}
...
leverage \foldalign \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{havgaard:2007ca} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gorodkin:1997tr,havgaard:2007ca} }%DIFAUXCMD
}\DIFaddend to measure the similarity between our
...
computationally predicted \grbs in {\em Bacillus megaterium}. \DIFaddbegin \DIFadd{We have already
...
bacterial species; \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend 12.0 currently contains 26 different families of aptamers
involved in different metabolic pathways.
\DIFaddbegin 

\DIFadd{Tools such as }\Rb \DIFadd{finder \mbox{%DIFAUXCMD
\citep{bengert2004} }%DIFAUXCMD
and }\ms{RiboSW}
\DIFadd{\mbox{%DIFAUXCMD
\citep{chang:2009de} }%DIFAUXCMD
have used the conserved
structural characteristics of the aptamer as search criteria with promising
results, while the webserver }\ms{RibEx} \DIFadd{relies solely on sequence conservation
to detect putative aptamer domains in genomic data \mbox{%DIFAUXCMD
\citep{abreugoodger:2005hb}}%DIFAUXCMD
.
Meanwhile other groups have used covariance model (CM)-based approaches to find
aptamer domains, most notably }\ms{CMfinder} \DIFadd{\mbox{%DIFAUXCMD
\citep{yao2006} }%DIFAUXCMD
and Rfam
\mbox{%DIFAUXCMD
...
\DIFaddend Whereas there exists strong sequence and
...
SCFG-based approaches. For this reason databases such as \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend only contain the aptamer
...
The RefSeq data we used for analysis \DIFaddbegin \DIFadd{(downloaded on }\fnaRetrievalTime\DIFadd{)
}\DIFaddend contains 5,121 annotated bacterial genomes
...
\subsubsection{Detecting \DIFdelbegin \DIFdel{Aptamers }\DIFdelend \DIFaddbegin \DIFadd{aptamers }\DIFaddend with \infernal}
...
sequence and structural conservation. Using the purine aptamer MSA from \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend 12.0
...
\DIFaddbegin \DIFadd{(}\Figref{rfinder:aptamerDiagram}\DIFadd{)}\DIFaddend . In total, using
\infernal with these additional filters yields 1,280 guanine aptamers across 555
unique organisms\DIFdelbegin \DIFdel{(note: here and elsewhere I define a `unique organism' as having
a unique taxonomy ID)}\DIFdelend .

\DIFaddbegin \begin{figure}[!ht]
\centering
\includegraphics[width=.4\textwidth]{Figures/Ribofinder/NC_000964_3_2320197_2320054_APTAMER.pdf}
\caption[Diagram of the aptamer portion of the \Bsxpt \grb]{\DIFaddFL{Diagram of the aptamer portion of the }\Bsxpt \grb\DIFaddFL{, with annotations
...
generated using VARNA \mbox{%DIFAUXCMD
\citep{darty:2009gt}}%DIFAUXCMD
.}}
\label{fig:rfinder:aptamerDiagram}
\end{figure}

\DIFaddend \subsubsection{Detecting \DIFdelbegin \DIFdel{Expression Platforms }\DIFdelend \DIFaddbegin \DIFadd{expression platforms }\DIFaddend with \tthp}
...
available in \DIFdelbegin \DIFdel{PTT }\DIFdelend \DIFaddbegin \DIFadd{NCBI Protein Table (PTT) }\DIFaddend data. These terminator sequences canonically have a stable
...
\DIFdelendFL \DIFaddbeginFL \caption[Histogram displaying the distribution of genome sizes across RefSeq]{\DIFaddendFL Histogram displaying the distribution of genome sizes across the RefSeq
data analyzed, comprising 5,172 bacterial genomes. Genome size is shown using a
$\log_{10}$ scale\DIFaddbeginFL \DIFaddFL{, and appears to have a bimodal distribution}\DIFaddendFL .}
...
\subsubsection{Notation for \DIFdelbegin \DIFdel{Representing Abstract }\DIFdelend \DIFaddbegin \DIFadd{representing abstract }\DIFaddend RNA \DIFdelbegin \DIFdel{Shapes}\DIFdelend \DIFaddbegin \DIFadd{shapes}\DIFaddend }
...
additionally be represented using a context-free grammar such as \DIFaddbegin \DIFadd{the
following,}\footnote{\DIFadd{Note that in the grammar from }\eqnref{rfinder:strCfg}\DIFadd{,
$\bullet^{\theta}$ should be interpreted to mean $\theta$ repetitions
of the $\bullet$ symbol, where $\theta$ is generally taken to be 3.}}
\DIFadd{derived from \mbox{%DIFAUXCMD
\citep{fusy:2012ka}}%DIFAUXCMD
}\DIFaddend :

\begin{align}
\label{eq:rfinder:strCfg}
\DIFdelbegin \DIFdel{S \rightarrow S\,.\;|\;.\,S\;|\;(S)\;|\;SS\;|\;\epsilon
...
same abstract tree shape \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{voss:2006iq}}%DIFAUXCMD
}\DIFaddend . This is analogous to what
the Giegerich lab refers to as
their `type 5' structural abstraction using the \rshapes tool\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and can be
described using the following grammar \mbox{%DIFAUXCMD
\citep{Lorenz:2008gz}}%DIFAUXCMD
:
}

\begin{align}
\DIFadd{\label{eq:rfinder:shapeCfg}
\begin{split}
...
validated xpt \grb from \DIFdelbegin \DIFdel{Bacillus subtilis }\DIFdelend \DIFaddbegin {\em \DIFadd{Bacillus subtilis}} \DIFaddend subsp. subtilis str. 168
...
{\em B. subtilis} subsp. subtilis str. 168 (NC\_000964.3 2320197--2320054),
and corresponding gene `off' structure derived from \DIFdelbeginFL \DIFdelFL{crystallography analysis }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inline probing studies both }\DIFaddendFL in
\DIFdelbeginFL \DIFdelFL{complex with }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the presence and absence of }\DIFaddendFL guanine \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citep{breaker:riboswitch2}}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{mandal2003}}%DIFAUXCMD
}\DIFaddendFL . {\em (Bottom)} The
experimentally
...
\subsubsection{Constrained \DIFdelbegin \DIFdel{Folding }\DIFdelend \DIFaddbegin \DIFadd{folding }\DIFaddend to \DIFdelbegin \DIFdel{Predict Switch Structures}\DIFdelend \DIFaddbegin \DIFadd{predict switch structures}\DIFaddend }
\label{subsubsec:rfinder:consfold}

To restrict our search to unannotated \grbs, and further ensure that we are not
re-detecting sequences based off the \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend covariance model provided to \infernal,
we constrain our search to those RefSeq organisms not represented in the \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend seed
...
energies respectively \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mathews:1999jw}}%DIFAUXCMD
. We choose to disable dangles (}\ms{-d 0}\DIFadd{) and use
the Turner 1999 energy model (}\ms{-P rna\_turner1999.par}\DIFadd{)
based on visual inspection of the structures output by }\rfold \DIFadd{with
constraints---these flags appear to yield conformations most frequently consistent
with the known structures for the }\Bsxpt \grb\DIFaddend .
...
\infernal P1 stem \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin [\DIFaddend resp. 3 base pairs\DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin ] \DIFaddend in the gene `off' \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin [\DIFaddend resp. gene `on'\DIFdelbegin \DIFdel{)
}\DIFdelend \DIFaddbegin ]
\DIFaddend structure.

\DIFdelbegin \DIFdel{This difficulty with }%DIFDELCMD < \rfold %%%
\DIFdel{can be shown by using the constraint-produced
structures as exhaustive constraints themselves. All unpaired nucleotides in
}%DIFDELCMD < \strOff %%%
\DIFdel{and }%DIFDELCMD < \strOn %%%
\DIFdel{are notated by a `}%DIFDELCMD < \ms{x}%%%
\DIFdel{' and all base pairs by `}%DIFDELCMD < \ms{()}%%%
\DIFdel{' for
the 5' and 3' side of the pair respectively to form new constraints mask
}%DIFDELCMD < \strConst{off} %%%
\DIFdel{and }%DIFDELCMD < \strConst{on}%%%
\DIFdel{, having all bases' state explicitly specified. By
refolding all 577 candidate sequences with }%DIFDELCMD < \strConst{off} %%%
\DIFdel{and }%DIFDELCMD < \strConst{on} %%%
\DIFdel{using
the same options as before, only 463 (or 80.24\%) of the resulting structures from
}%DIFDELCMD < \strConst{off} %%%
\DIFdel{have the tree signature prefix }%DIFDELCMD < \ms{[0,1,2,2,1]}%%%
\DIFdel{, and just 21 (or
3.64\%) of the }%DIFDELCMD < \strConst{on} %%%
\DIFdel{structures correctly re-fold their multiloop.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Step 3: Candidate curation}
...
\foldalign \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{havgaard:2007ca} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gorodkin:1997tr,havgaard:2007ca} }%DIFAUXCMD
}\DIFaddend alongside the \ms{needleall} tool from EMBOSS
...
\DIFdelbegin \DIFdel{experimental constraints}\DIFdelend \DIFaddbegin \DIFadd{expense}\DIFaddend , we elected \DIFdelbegin \DIFdel{finally }\DIFdelend to instead choose a small number
...
is based off \DIFaddbegin \DIFadd{a simplification of }\DIFaddend Sankoff's algorithm \citep{sankoff:1985wc}, \DIFaddbegin \DIFadd{and
is }\DIFaddend a dynamic programming
algorithm for simultaneous folding and alignment that runs in \DIFdelbegin %DIFDELCMD < \On{6} %%%
\DIFdel{time and
}\DIFdelend \On{4} \DIFdelbegin \DIFdel{space}\DIFdelend \DIFaddbegin \DIFadd{time}\DIFaddend . Because three of the sequences from our pool of 503 candidates have
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Histogram displaying the distribution of scores produced by \foldalign]{\DIFaddendFL Histogram displaying the distribution of scores produced by \foldalign
...
overseeing \DIFaddbegin \DIFadd{Dr. Regula Aregger, }\DIFaddend a post-doc who is using the SHAPE protocol
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Histogram displaying the number of candidate \rbs we observe
per organism]{\DIFaddendFL Histogram displaying the \DIFdelbeginFL \DIFdelFL{distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{number }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{scores produced by }%DIFDELCMD < \foldalign
%DIFDELCMD < %%%
\DIFdelFL{2.1.1 using flags }%DIFDELCMD < \ms{-global -summary -format commandline} %%%
\DIFdelFL{when folding each of
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{candidate }\rbs \DIFaddFL{we observe
per organism. From this data, it is clear that }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{503 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{majority of organisms have
only one putative }\rb\DIFaddFL{, however two organisms have five }\DIFaddendFL candidates \DIFdelbeginFL \DIFdelFL{against the }%DIFDELCMD < \Bsxpt %%%
\DIFdelFL{sequence NC\_000964.3 2320197--2320054}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each:
}{\em \DIFaddFL{Clostridium botulinum}} \DIFaddFL{B str}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Three of the sequences run against }%DIFDELCMD < \foldalign %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Eklund 17B }\DIFaddendFL (NC\_010674.1\DIFdelbeginFL \DIFdelFL{1516712--1516868,
NC\_010723.1 1487041--1487197, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL and
\DIFaddbeginFL {\em \DIFaddFL{Clostridium botulinum}} \DIFaddFL{E3 str. Alaska E43 (}\DIFaddendFL NC\_\DIFdelbeginFL \DIFdelFL{020291.1 4599412--4599258}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{010723.1}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{have no global
alignment with the }%DIFDELCMD < \Bsxpt %%%
\DIFdelFL{sequence, and thus the histogram represents 500 of the
original 503 sequences.}\DIFdelendFL }
...
{\em B. megaterium} QM B1551 and {\em B. megaterium} DSM319]{\DIFaddendFL The genomic coordinates for the four candidate \grbs in both
...
termination as a mode of \DIFdelbegin \DIFdel{regulation, for }\DIFdelend \DIFaddbegin \DIFadd{regulation---for }\DIFaddend which an aptamer alignment exists and
some experimental knowledge of the \DIFdelbegin \DIFdel{terminator stem}\DIFdelend \DIFaddbegin \DIFadd{expression platform}\DIFaddend 's structural
conformation \DIFdelbegin \DIFdel{are
}\DIFdelend \DIFaddbegin \DIFadd{is known---are }\DIFaddend well suited for more robust structural
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the xpt gene in {\em B. megaterium} QM B1551]{\DIFaddendFL {\em Top:} the computationally predicted gene `off' conformation of
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the GMP synthase gene in {\em B. megaterium} QM B1551]{\DIFaddendFL The computationally predicted \rb located upstream of the GMP synthase
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the guanine permease gene in {\em B. megaterium} QM B1551]{\DIFaddendFL The computationally predicted \rb located upstream of the guanine permease
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the N5- carboxy\-amino\-imidazole gene in {\em B. megaterium} QM B1551]{\DIFaddendFL The computationally predicted \rb located upstream of the
\DIFdelbeginFL \DIFdelFL{N5-carboxyaminoimidazole
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{N5- carboxy\-amino\-imidazole
}\DIFaddendFL gene in {\em B. megaterium} QM B1551 (NC\_014019.1 240759--240970).
{\em Top:} The gene `off' conformation. {\em Bottom:} The gene `on' conformation.}
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the xpt gene in {\em B. megaterium} DSM319]{\DIFaddendFL The computationally predicted \rb located upstream of the xpt
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the GMP synthase gene in {\em B. megaterium} DSM319]{\DIFaddendFL The computationally predicted \rb located upstream of the GMP synthase
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the guanine permease gene in {\em B. megaterium} DSM319]{\DIFaddendFL The computationally predicted \rb located upstream of the guanine permease
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Structures for the putative \rb located upstream of the N5- carboxy\-amino\-imidazole gene in {\em B. megaterium} DSM319]{\DIFaddendFL The computationally predicted \rb located upstream of the
\DIFdelbeginFL \DIFdelFL{N5-carboxyaminoimidazole
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{N5- carboxy\-amino\-imidazole
}\DIFaddendFL gene in {\em B. megaterium} DSM319 (NC\_014103.1 239188--239400).
{\em Top:} The gene `off' conformation. {\em Bottom:} The gene `on' conformation.}
...
the Boltzmann probability of those structures \DIFdelbegin \DIFdel{whom}\DIFdelend \DIFaddbegin \DIFadd{which}\DIFaddend , for a given input RNA
...
reformulated as a polynomial whose coefficients \DIFdelbegin \DIFdel{$c_k=\bfZ{k}{1,n}$}\DIFdelend \DIFaddbegin \DIFadd{$z_k=\bfZ{k}{1,n}$}\DIFaddend . We then
describe how the \fft can be employed to efficiently compute the coefficients
\DIFdelbegin \DIFdel{$c_k$}\DIFdelend \DIFaddbegin \DIFadd{$z_k$}\DIFaddend , finishing our description of the underlying algorithm. Then we proceed
...
whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend to a given {\em initial}, or
...
sum of all Boltzmann factors of all structures \str, whose \DIFdelbegin \DIFdel{base pair distance
}\DIFdelend \DIFaddbegin \bpd
\DIFaddend to the initial structure \strSt is exactly $k$.}
...
{\em provided} that the starting structure \strSt is the \DIFdelbegin \DIFdel{minimum free
energy }\DIFdelend \DIFaddbegin \mfe
\DIFaddend (MFE) structure. We expect that a more efficient version of
...
\DIFdelbegin \DIFdel{Base pair distance }\DIFdelend \DIFaddbegin \Bpd \DIFaddend is defined as the symmetric difference between the sets
\strST:
...
concern the Turner energy model \DIFaddbegin \DIFadd{(described in }\Secref{sec:fftbor:turner}\DIFadd{)}\DIFaddend , consisting of free energy parameters for
...
\fullZx = \sum_{k=0}^n \DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{z}\DIFaddend _k x^k
\end{align}

where coefficients \DIFdelbegin \DIFdel{$c_k=\bfZ{k}{1,n}$}\DIFdelend \DIFaddbegin \DIFadd{$z_k=\bfZ{k}{1,n}$}\DIFaddend . Moreover, because of
\eqnref{fftbor:maxDist} and the fact that the minimum number of
unpaired bases in a hairpin loop $\theta$ is 3, we know that \DIFdelbegin \DIFdel{$c_n=0$}\DIFdelend \DIFaddbegin \DIFadd{$z_n=0$}\DIFaddend ,
...
we can compute the coefficients \DIFdelbegin \DIFdel{$c_k$ }\DIFdelend \DIFaddbegin \DIFadd{$z_k$ }\DIFaddend of polynomial \fullZx. As we describe
...
\DIFdelbegin \DIFdel{$c_k=\bfZ{k}{1,n}$ }\DIFdelend \DIFaddbegin \DIFadd{$z_k=\bfZ{k}{1,n}$ }\DIFaddend can be computed after
...
\bfZ{k}{i,j} = \bfZ{k-b_0}{i,j-1}\enspace +
\sum\DIFdelbegin \DIFdel{_{\substack{s_r s_j \in \bpSet, \\ i \le r<j}}
}\DIFdelend \DIFaddbegin \DIFadd{_{\substack{(s_r,s_j) \in \bpSet, \\ i \le r<j}}
}\DIFaddend \left(
...
\emZ{i,j} = \sum_{k=0}^n \DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{z}\DIFaddend _k(i,j)\,x^k
\end{align}

where

\begin{align}
\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{z}\DIFaddend _k(i,j)\>=\>
...
\begin{theorem}
\DIFaddbegin \label{thm:fftbor:recursions}
\DIFaddend Let \seqN be a given RNA sequence.
For any integers $1 \leq i \leq j \leq n$, let

\begin{align}
\emZ{i,j} = \sum_{k=0}^n \DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{z}\DIFaddend _k\,x^k
\end{align}

where

\begin{align}
\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{z}\DIFaddend _k(i,j) = \bfZ{k}{i,j}.
\end{align}
...
polynomial [resp. analytic] function, then \DIFdelbegin \DIFdel{$[x^k] F(x)$
}\DIFdelend \DIFaddbegin \DIFadd{$[x^k]F(x)$
}\DIFaddend denotes the coefficient of $x^k$ [resp. the $k$th Taylor coefficient in the
Taylor expansion of \DIFdelbegin \DIFdel{$F$}\DIFdelend \DIFaddbegin \DIFadd{$F(x)$}\DIFaddend ]. For instance, in
\eqnref{fftbor:pOfX}, $[x^k]p(x) = \pk$, and in
\eqnref{fftbor:zOfX}, \DIFdelbegin \DIFdel{$[x^k]\fullZx = c_k(i,j)$}\DIFdelend \DIFaddbegin \DIFadd{$[x^k]\fullZx = z_k$}\DIFaddend .
...
%DIFDELCMD < [x^k] \emZ{i,j} &= c_k(i,j) = \bfZ{k}{i,j} \\
%DIFDELCMD < &= \bfZ{k-b_0}{i,j-1} +
%DIFDELCMD < \sum_{r=i}^{j-1}\hspace{2.25em} \sum_{\mathclap{k_0+k_1=k-b(r)}}\enspace
%DIFDELCMD < \boltzNuss{r,j} \cdot \bfZ{k_0}{i,r-1} \cdot \bfZ{k_1}{r+1,j-1} \\
%DIFDELCMD < &= [x^{k-b_0}] \emZ{i,j-1} \\
%DIFDELCMD < &+ \sum_{r=i}^{j-1}\hspace{2.25em} \sum_{\mathclap{k_0+k_1=k-b(r)}}\enspace
%DIFDELCMD < \boltzNuss{r,j} \cdot \left\{ [x^{k_0}] \emZ{i,r-1} \right\} \cdot
%DIFDELCMD < \left\{ [x^{k_1}] \emZ{r-1,j-1} \right\} \\
%DIFDELCMD < &= [x^{k-b_0}] \emZ{i,j-1} \\
%DIFDELCMD < &+ \sum_{r=i}^{j-1}\hspace{2.25em} \sum_{\mathclap{k_0+k_1=k-b(r)}}\enspace
%DIFDELCMD < \boltzNuss{r,j} \cdot [x^{k_0+k_1}] \emZ{i,r-1} \emZ{r-1,j-1} .\\
%DIFDELCMD < \end{split}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{split}
...
\DIFdelbegin \DIFdel{$c_k=\bfZ{k}{1,n}$ }\DIFdelend \DIFaddbegin \DIFadd{$z_k=\bfZ{k}{1,n}$ }\DIFaddend in the polynomial
\DIFdelbegin \DIFdel{$\fullZx = c_0 + c_1 x + \dots + c_{n-1} x^{n-1}$ }\DIFdelend \DIFaddbegin \DIFadd{$\fullZx = z_0 + z_1 x + \dots + z_{n-1} x^{n-1}$ }\DIFaddend defined in
...
\end{algorithmic}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Pseudocode for \fftbor]{\DIFaddendFL The function {\sc FFTbor} computes the $m$ most significant digits
...
Recall that \DIFdelbegin \DIFdel{$\fullZx = \sum_{k=0}^n c_k x^k$, where
$c_k \in \mathbb{R}$ }\DIFdelend \DIFaddbegin \DIFadd{$\fullZx = \sum_{k=0}^n z_k x^k$, where
$z_k \in \mathbb{R}$ }\DIFaddend are real numbers representing the partition function
\bfZ{k}{1,n} over
all secondary structures of a given RNA sequence \seqN,
whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend from initial structure
...
$(\aMbi)^m$. Since \emZof{}{\aPbi} is a sum of terms of the form \DIFdelbegin \DIFdel{$c_k (\aPbi)^k$}\DIFdelend \DIFaddbegin \DIFadd{$z_k (\aPbi)^k$}\DIFaddend ,
...
where the $x$-axis represents the \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend from an arbitrary,
...
has \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend $k$ from \strSt. Arguably, this probability distribution
...
Rfam consensus and \DIFdelbegin \DIFdel{minimum free energy structures }\DIFdelend \DIFaddbegin \mfess \DIFaddend for both sequences are
...
consensus and
\DIFdelbegin \DIFdel{minimum free energy structures }\DIFdelend \DIFaddbegin \mfess \DIFaddend closely resemble each other, and that the
...
denote the Rfam consensus structure \DIFaddbegin [\DIFaddend resp. MFE structure\DIFaddbegin ] \DIFaddend for the 97 nt
...
denote the Rfam consensus structure \DIFaddbegin [\DIFaddend resp. MFE structure\DIFaddbegin ] \DIFaddend for the 99 nt
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Rfam consensus structures (Rfam) and \mfe (MFE)
secondary structures for two thiamine pyrophosphate (TPP) \rb aptamers]{\DIFaddendFL Rfam consensus structures (Rfam) and \DIFdelbeginFL \DIFdelFL{minimum free energy }\DIFdelendFL \DIFaddbeginFL \mfe \DIFaddendFL (MFE)
...
{\em (A)} \DIFdelbeginFL \DIFdelFL{Rfam consensus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MFE }\DIFaddendFL structure for BX842649.1/277414--277318.
{\em (B)} \DIFdelbeginFL \DIFdelFL{MFE }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Rfam consensus }\DIFaddendFL structure for BX842649.1/277414--277318.
{\em (C)} \DIFdelbeginFL \DIFdelFL{Rfam consensus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MFE }\DIFaddendFL structure for AACY022101973.1/389--487.
...
\begin{figure}[!ht]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{Figures/FFTbor/tppDistributions.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.45\textwidth]{Figures/FFTbor/FFTbor_AACY022101973_1.pdf}
\quad
...
thiamine pyrophosphate \rb (TPP) aptamers]{\DIFaddendFL Output from \fftbor on two randomly selected
thiamine pyrophosphate \rb (TPP) aptamers, taken from the Rfam database
\citep{Gardner.nar11}. The $x$-axis represents \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL from the
\DIFdelbeginFL \DIFdelFL{minimum free energy structure }\DIFdelendFL \DIFaddbeginFL \mfes \DIFaddendFL for each given sequence; the $y$-axis represents
...
whose \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL from the MFE structure is exactly $k$.
{\em (Left)}
The 97 nt sequence BX842649.1/277414--277318 appears to have a rugged energy
landscape near its \DIFdelbeginFL \DIFdelFL{minimum free energy structure}\DIFdelendFL \DIFaddbeginFL \mfes\DIFaddendFL , with distinct
...
taken by \kinfold\DIFdelbegin \DIFdel{, each }\DIFdelend \DIFaddbegin \DIFadd{---each }\DIFaddend step consisting of the addition or removal
of a single base \DIFdelbegin \DIFdel{pair (or shift --- see \mbox{%DIFAUXCMD
\citep{flamm}}%DIFAUXCMD
), to }\DIFdelend \DIFaddbegin \DIFadd{pair---to }\DIFaddend fold the
...
500,000 for the 10 runs that did not converge, we found a \DIFdelbegin \DIFdel{mean first
passage time }\DIFdelend \DIFaddbegin \mfpt
\DIFaddend of $311,075.06$ steps for this sequence.
The second sequence, AACY022101973.1/389--487, converged within 500,000
steps in 29 out of 30 runs, and we found a \DIFdelbegin \DIFdel{mean first passage time }\DIFdelend \DIFaddbegin \mfpt \DIFaddend of
...
$\seq = \seqN$.
The expected \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend to \strSt is defined by
...
\dBP{\str}{\strSt} denotes \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend between \str and \strSt.
...
\DIFaddbegin \DIFadd{Surprisingly, we found that there is a significant
correlation of $0.4847$ with one-tailed
$p$-value of $0.0002$ between the
standard deviation of the }\fftbor \DIFadd{output (when starting from the
empty structure) and logarithm base 10 of the }\mfpt\DIFadd{.
}\DIFaddend As described above, \fftbor output is simply
the probability distribution
for the number of base pairs per structure, taken over the ensemble
of all secondary structure for the input RNA
sequence. \DIFdelbegin \DIFdel{Surprisingly, we found that there is a significant
correlation of  $0.48436192$ with one-tailed
$p$-value of $0.00018249$ between the
}\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend standard deviation of \DIFdelbegin \DIFdel{the }%DIFDELCMD < \fftbor %%%
\DIFdel{output (when starting from the
empty structure) and logarithm base 10 of the mean first passage time}\DIFdelend \DIFaddbegin \DIFadd{this probability distribution corresponds
to a notion of the }{\em \DIFadd{width}} \DIFadd{of the distribution. It is possible that those
...
due to other local minima causing the RNA to get trapped while folding}\DIFaddend .
...
RF00031]{\DIFaddendFL \small Pearson correlation between various aspects of selenocysteine
insertion sequences from the seed alignment of Rfam family
...
{\em 5})\, the \DIFdelbeginFL \DIFdelFL{minimum free energy }\DIFdelendFL \DIFaddbeginFL \mfe \DIFaddendFL (MFE).
...
{\em iii})\;
the mean $\mu$ is negatively correlated with the
\DIFdelbeginFL \DIFdelFL{minimum free energy }\DIFdelendFL \DIFaddbeginFL \mfe \DIFaddendFL (MFE) --- if most low energy structures in the ensemble
have many base pairs, then it is likely that the \DIFdelbeginFL \DIFdelFL{minimum free energy }\DIFdelendFL \DIFaddbeginFL \mfe \DIFaddendFL is very
...
increases, the \DIFdelbeginFL \DIFdelFL{minimum free energy }\DIFdelendFL \DIFaddbeginFL \mfe \DIFaddendFL (MFE) decreases.
However, it may appear surprising that
{\em v})\; the
mean $\mu$ number of base pairs per structure is independent of MFPT
(correlation \DIFdelbeginFL \DIFdelFL{$-0.036291124$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$-0.0363$}\DIFaddendFL ), although
{\em vi})\; MFE is correlated with MFPT
(correlation \DIFdelbeginFL \DIFdelFL{$0.399015556$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.3990$}\DIFaddendFL ) --- i.e. from {\em (iii)},
...
{\em vii})\;
standard deviation $\sigma$ is correlated with \DIFdelbeginFL \DIFdelFL{mean first passage
time --- the }\DIFdelendFL \DIFaddbeginFL \mfpt\DIFaddFL{---the }\DIFaddendFL correlation is statistically significant, with one-tailed
$p$-value of \DIFdelbeginFL \DIFdelFL{$0.00018249$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0002$}\DIFaddendFL .}
...
\begin{figure}[!ht]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{Figures/FFTbor/correlationFFTborEmpty.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.3\textwidth]{Figures/FFTbor/AB030643_1_4176-4241.pdf}
...
\caption[Example of the graphical output of \fftbor, when the empty structure is chosen as initial structure \strSt]{\DIFaddendFL This figure represents the
...
\DIFdelbeginFL \DIFdelFL{$0.727618$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.7276$}\DIFaddendFL , while the logarithm base 10 of the \DIFdelbeginFL \DIFdelFL{mean first passage time }\DIFdelendFL \DIFaddbeginFL \mfpt \DIFaddendFL ($\log_{10}(\text{MFPT})$)
is $4.75$.
{\em (Center)}
...
RF00031, the standard deviation $\sigma$ of the number of base pairs,
taken over the ensemble of all secondary structures, is
\DIFdelbeginFL \DIFdelFL{$2.679446$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2.6794$}\DIFaddendFL , while $\log_{10}(\text{MFPT})$ is $5.69$.
...
is \DIFdelbeginFL \DIFdelFL{$2.22528791$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2.2253$  }\DIFaddendFL [resp. \DIFdelbeginFL \DIFdelFL{$1.98565959$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.9857$}\DIFaddendFL ], and the \kinfold MFPT is
...
standard deviation $\sigma$ for the \fftbor structural profile
is \DIFdelbegin \DIFdel{$\sigma_1=2.22528791$  }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_1=2.2253$  }\DIFaddend [resp. \DIFdelbegin \DIFdel{$\sigma_2=1.98565959$}\DIFdelend \DIFaddbegin \DIFadd{$\sigma_2=1.9857$}\DIFaddend ], and the \kinfold MFPT is
$311,075.06$ [resp. $61,575.69$] for the TPP \rb aptamer
...
found
that for randomized BX842649, all 30 runs converged yielding a \DIFdelbegin \DIFdel{mean first passage
time }\DIFdelend \DIFaddbegin \mfpt
\DIFaddend (MFPT) of $13,022.58$ with standard deviation of $15,221.78$. In contrast for
randomized AACY022101973, only 15 out of 30 runs converged within 500,000 steps,
and discounting these nonconvergent data, we obtain an average \DIFdelbegin \DIFdel{mean first passage
time }\DIFdelend \DIFaddbegin \mfpt
\DIFaddend (MFPT) of $94,446.93$ with standard deviation of $157,107.43$. This additional
...
%DIF <  \section{Riboswitch detection with \fftbor}
%DIF <  \label{sec:fftbor:rb}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  A bacterial \rb is a portion of the $5'$
%DIF <  untranslated region (UTR) of messenger RNA,
%DIF <  that performs gene regulation by undergoing a conformational change upon
%DIF <  binding with a ligand, such as guanine, thiamine pyrophosphate, lysine,
%DIF <  etc.  \citep{breaker:Riboswitch2}.
%DIF <  This conformational change may either turn  on or off
%DIF <  the corresponding gene by either transcriptional or translational regulation
%DIF <  of the messenger RNA \citep{tuckerBreaker:review}, depending on the
%DIF <  particular \rb. The common feature shared by all \rbs
%DIF <  is that a gene is regulated by conformational change upon ligand binding.
%DIF <  Bacterial \rbs are often found upstream
%DIF <  of operons, regulating groups of genes, as in purine {\em de novo}
%DIF <  synthesis and salvage \citep{mandalboesebarrickwinklerbreaker}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  A \rb consists of two equally important parts: an upstream
%DIF <  {\em aptamer}, capable of highly discriminative binding to a particular
%DIF <  ligand, and a downstream {\em expression platform}, capable of undergoing
%DIF <  a radical conformational change upon binding of a ligand with the
%DIF <  discriminating aptamer.
%DIF <  Since aptamers have been under strong evolutionary pressure to
%DIF <  bind with high affinity
%DIF <  (e.g. $K_D \approx 5$ nM for guanine \citep{Mandal.nsmb04}),
%DIF <  there is strong
%DIF <  sequence conservation found in the aptameric region of orthologous
%DIF <  \rbs.
%DIF <  In contrast, while secondary structure is conserved in the
%DIF <  terminator loop of the expression platform in purine \rbs,
%DIF <  there is relatively low sequence conservation (data not shown).
%DIF <  While a number of methods exist to computationally predict
%DIF <  \rb aptamers
%DIF <  \citep{Bengert2004Riboswitch,Chang2009Computational,AbreuGoodger2005RibEx,Singh2009Riboswitch,Bergig.isb04}
%DIF <  (and especially {\tt INFERNAL} \citep{infernal}, which latter is used
%DIF <  to predict \rb aptamers in Rfam), it is an important
%DIF <  biological problem to determine the expression platform, since the structure
%DIF <  of the expression platform can suggest whether there is
%DIF <  transcriptional regulation
%DIF <  via a terminator loop or translational regulation via the sequestration of
%DIF <  the Shine-Dalgarno sequence \citep{tuckerBreaker:review}. Determination of
%DIF <  the precise location and structure of the expression platform is difficult
%DIF <  due to low conserved sequence identity (in-house computations, data not shown).
%DIF <  Although this problem remains open, we report here how
%DIF <  \fftbor may provide help to biologists in the selection and
%DIF <  prioritization of \rb candidates.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Figure 5 depicts the gene OFF structure of the
%DIF <  xpt G-box purine \rb in {\em B. subtilis}, as determined by
%DIF <  inline-probing --- this structure was taken from
%DIF <  Figure 1 of \citep{breaker:Riboswitch2}.
%DIF <  Note that this structure is only partial, since there are regions
%DIF <  with no base pairs depicted, despite the fact that additional
%DIF <  base pairs could be added. By using
%DIF <  {\tt blastn}, it is found that this 161 nt purine \rb
%DIF <  can be found on the complement strand of GenBank accession number
%DIF <  CP002906.1/2165302--2165142 in
%DIF <  complete genome of {\em B. subtilis subsp. subtilis RO-NN-1}.
%DIF <  Figure 6 depicts the result of
%DIF <  three computational experiments with FFTbor. The left panel displays
%DIF <  the expected base pair distance to the following secondary structure
%DIF <  \begin{small}
%DIF <  \begin{eqnarray}
%DIF <  \label{eq:xpt1}
%DIF <  %ACACUCAUAUAAUCGCGUGGAUAUGGCACGCAAGUUUCUACCGGGCACCGUAAAUGUCCGACUAUGGGUGAGCAAUGGAACCGCACGUGUACGGUUUUUUGUGAUAUCAGCAUUGCUUGCUCUUUAUUUGAGCGGGCAAUGCUU
%DIF <  \bf
%DIF <  .((((((((...(((((((.......)))))))........((((((.......))))))..))))))))........(((((........)))))............((((((((((((((.......)))))))))))))).
%DIF <  \end{eqnarray}
%DIF <  \end{small}
%DIF <  as a function of window offset, where window size equals the size of this
%DIF <  target structure. This structure was
%DIF <  obtained by removing all leading and trailing unpaired positions
%DIF <  from the structure depicted in Figure 5,
%DIF <  except for the leftmost [resp. rightmost] unpaired position
%DIF <  adjacent to the leftmost [resp. rightmost] base-paired position.
%DIF <  The reason for removal of the leading and trailing unpaired positions was
%DIF <  that the structure of \citep{breaker:Riboswitch2}, depicted in
%DIF <  Figure 5, is clearly only partial, as earlier
%DIF <  mentioned.  The center panel displays
%DIF <  the expected base pair distance to the following secondary structure
%DIF <  \begin{small}
%DIF <  \begin{eqnarray}
%DIF <  \label{eq:xpt2}
%DIF <  %ACACUCAUAUAAUCGCGUGGAUAUGGCACGCAAGUUUCUACCGGGCACCGUAAAUGUCCGACUAUGGGUGA
%DIF <  \bf
%DIF <  .((((((((...(((((((.......)))))))........((((((.......))))))..)))))))).
%DIF <  \end{eqnarray}
%DIF <  \end{small}
%DIF <  as a function of window offset, where window size equals the size of this
%DIF <  target {\em aptamer} structure. Similarly, the right panel displays
%DIF <  the expected base pair distance to the following secondary structure
%DIF <  \begin{small}
%DIF <  \begin{eqnarray}
%DIF <  \label{eq:xpt3}
%DIF <  %GAACCGCACGUGUACGGUUUUUUGUGAUAUCAGCAUUGCUUGCUCUUUAUUUGAGCGGGCAAUGCUU
%DIF <  \bf
%DIF <  .(((((........)))))............((((((((((((((.......)))))))))))))).
%DIF <  \end{eqnarray}
%DIF <  \end{small}
%DIF <  as a function of window offset, where window size equals the size of this
%DIF <  target {\em expression platform} structure.
%DIF <  Figure 6 determines the precise location of the
%DIF <  xpt \rb, both aptamer and expression platform.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  If the biologically functional target structure is unknown, one can
%DIF <  instead attempt a similar moving window computation, where the target
%DIF <  structure is taken to be the minimum free energy structure of the
%DIF <  current window contents. In this case, one may hope to determine a
%DIF <  bimodal distribution, as displayed in Figure 7.
%DIF <  Given an input RNA sequence, or genomic region, the
%DIF <  web server \url{http://bioinformatics.bc.edu/clotelab/FFTbor}
%DIF <  creates a movie as follows, described here for the xpt \rb
%DIF <  previously discussed.  We extracted the downstream region of
%DIF <  Taking the
%DIF <  161 nt xpt G-box purine \rb described in Figure 5,
%DIF <  with GenBank accession number CP002906.1/2165302--2165142, we extended
%DIF <  to a sequence of length 200 nt, by appending flanking
%DIF <  downstream genomic nucleotides.  Running \fftbor on all
%DIF <  prefixes of the resulting sequence of lengths
%DIF <  $70,72,74,\ldots,200$, we produced a movie, displayed on the webserver
%DIF <  \url{http://bioinformatics.bc.edu/clotelab/FFTbor}.
%DIF <  Figure 5 displays the output of \fftbor on the
%DIF <  166 nt prefix, clearly showing a bimodal distribution. Attempting to
%DIF <  automate the identification of non-unimodal \fftbor output, we have
%DIF <  applied the Hartigan-Hartigan dip-test \citep{hartigandiptest},
%DIF <  implemented in {\tt R}; however,
%DIF <  the dip-test appears to be too sensitive, in that a probability distribution
%DIF <  is reported to be non-unimodal, even when visual inspection indicates that
%DIF <  it appears overwhelmingly to be unimodal (data not shown). It is for this
%DIF <  reason that the web server
%DIF <  \url{http://bioinformatics.bc.edu/clotelab/FFTbor} produces a movie of
%DIF <  prefixes, where the user can start/stop the movie,
%DIF <  move forward/backward, or download all raw data output by \fftbor.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Performance characteristics of \fftbor and \rnabor}
...
\DIFaddbegin \DIFadd{While }\fftbor \DIFadd{saves an order of magnitude in performance and memory,
}\rnabor \DIFadd{has the benefit of producing suboptimal structures $\text{MFE}_k$
for all $0 \leq k \leq n$, whose free energy is minimal across all structures
having }\bpd \DIFadd{$k$ from input structures }\strSt\DIFadd{.
}\DIFaddend \Figref{fftbor:benchmarking} displays run time curves for both
\rnabor and \fftbor, when the initial structure \strSt is
taken to be either the empty structure or the \DIFdelbegin \DIFdel{minimum free energy
}\DIFdelend \DIFaddbegin \mfe
\DIFaddend (MFE) structure.
...
\begin{figure}[!ht]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{Figures/FFTbor/benchmarking.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.9\textwidth]{Figures/FFTbor/rnaborfftborRunTimeEvan.pdf}
\caption[Run times in seconds for \rnabor and \fftbor, on random RNA
of length $20,40,60,\dots,300$ in step size of 20 nt]{\DIFaddendFL Run times in seconds for \rnabor and \fftbor, on random RNA
of length $20,40,60,\dots,300$ in step size of 20 nt. Each algorithm
was run with the empty initial structure \strSt, see rows
\rnabor (empty), \fftbor (empty), and with the \DIFdelbeginFL \DIFdelFL{minimum free
energy structure }\DIFdelendFL \DIFaddbeginFL \mfes \DIFaddendFL as the initial structure
\strSt, see rows
...
The \DIFdelbegin \DIFdel{left panel of }%DIFDELCMD < \Figref{fftbor:benchmarkingParallel} %%%
\DIFdelend \DIFaddbegin \DIFadd{data in Table \ref{table:fftborBenchmarkingParallel} }\DIFaddend describes average
...
\DIFdelbegin \DIFdel{The right panel of }\DIFdelend \Figref{fftbor:benchmarkingParallel}
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{Figures/FFTbor/benchmarkingOpenMP.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \begin{tabularx}{\linewidth}{c *{2}{R}}
\toprule
\small{$n$} & \small{Single core} & \small{Two cores} \\
\cmidrule\DIFaddFL{(lr)}\DIFaddendFL {\DIFdelbeginFL %DIFDELCMD < \em %%%
\DIFdelFL{(Left)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1-3}\DIFaddendFL }
\DIFdelbeginFL \DIFdelFL{Table showing parallel run times in seconds
for }%DIFDELCMD < \fftbor%%%
\DIFdelFL{, using
OpenMP---}%DIFDELCMD < \url{http://openmp.org/}%%%
\DIFdelFL{. Column headers }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{200 }& \DIFaddFL{123.2 $\pm$ 16.2 }& \DIFaddFL{61.8 $\pm$ 8.0 }\\
\DIFaddFL{250 }& \DIFaddFL{331.1 $\pm$ 27.2 }& \DIFaddFL{166.1 $\pm$ 13.7 }\\
\DIFaddFL{300 }& \DIFaddFL{723.4 $\pm$ 59.9 }& \DIFaddFL{365.2 $\pm$ 30.1 }\\
\DIFaddFL{350 }& \DIFaddendFL 1\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{,380.8 $\pm$ 95.2 }& \DIFaddFL{698.4 $\pm$ 46.9 }\\
\DIFaddFL{400 }& \DIFaddendFL 2\DIFdelbeginFL \DIFdelFL{indicate
the number of cores used in the computational experiment.
For each sequence length $200,\dots,500$}\DIFdelendFL ,\DIFdelbeginFL \DIFdelFL{five random RNAs were generated using equal probability for each nucleotide
A}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{239.1 $\pm$ 210.9 }& \DIFaddFL{1}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{C}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{129.5 $\pm$ 104.3 }\\
\DIFaddFL{450 }& \DIFaddFL{3}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{G}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{635.0 $\pm$ 857.4 }& \DIFaddFL{1}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{U. Run time in seconds}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{980.9 $\pm$ 126.5 }\\
\DIFaddFL{500 }& \DIFaddFL{5}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{plus or minus one standard deviation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{076.7 $\pm$ 1}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{are
given for a 24-core
AMD Opteron 6172 with 2.10GHz and 64GB RAM}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{292.1 }& \DIFaddFL{3}\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{with only 1 (resp. 2) cores
used.
}%DIFDELCMD < {\em %%%
\DIFdelFL{(Right)}%DIFDELCMD < } %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{389.8 $\pm$ 788.4 }\\
\bottomrule
\end{tabularx}
\caption[Table showing parallel run times in seconds
for \fftbor, using OpenMP]{\DIFaddFL{Table showing parallel run times in seconds
for }\fftbor\DIFaddFL{, using OpenMP---}\url{http://openmp.org/}\DIFaddFL{.
...
given for a 24-core
AMD Opteron 6172 running at 2.10GHz with 64GB RAM, with only 1 }[\DIFaddFL{resp. 2}] \DIFaddFL{cores
used.}}
...
\caption[]{\DIFaddendFL Graph showing parallel run time of \fftbor \DIFaddbeginFL \DIFaddFL{as a function of
sequence length, running }\DIFaddendFL on an AMD Opteron 6172 \DIFdelbeginFL \DIFdelFL{with }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{running at }\DIFaddendFL 2.10GHz \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with }\DIFaddendFL 64GB RAM,
...
the \idft, and map to the 2D energy landscape. We describe two \DIFdelbegin \DIFdel{exploitations of }\DIFdelend \DIFaddbegin \DIFadd{improvements over
the straight-forward use of the }\fft \DIFadd{to compute }\DIFaddend $P(x)$,
a parity condition and complex conjugates\DIFdelbegin \DIFdel{which further }\DIFdelend \DIFaddbegin \DIFadd{, which together }\DIFaddend reduce the
...
has been done on folding pathways\DIFaddbegin \DIFadd{. In \mbox{%DIFAUXCMD
\citep{lorenz}}%DIFAUXCMD
, }\rnatwofold\DIFadd{---a dynamic
programming method that operates in
}\On{7} \DIFadd{time and }\On{4} \DIFadd{space---is presented, a generalization of }\rnabor
\DIFadd{\mbox{%DIFAUXCMD
\citep{freyhult.b07} }%DIFAUXCMD
that computes a 2D projection of the energy landscape,
discretized by }\bpd \DIFadd{to two input structures. Our work presented
here, called }\ffttwo\DIFadd{, produces a similar energy landscape in }\On{5} \DIFadd{time }\DIFaddend and
\DIFdelbegin \DIFdel{the kinetics of RNA folding;
below, we cite only a small sample of the work in this area.
On the experimental side,
%DIF < Baird et al.  \citep{Baird.jacs10} used cryo-electron microscopy and single
%DIF < particle image reconstruction to determine the structure of the major folding
%DIF < intermediate of the specificity domain of a ribonuclease P ribozyme.
%DIF < Mitra et al.  \citep{Mitra.r11} conducted experiments using
%DIF < time-resolved hydroxyl radical probing of backbone solvent accessible
%DIF < surface and catalytic activity measurements, integrated with
%DIF < structural-kinetic modeling to compare folding pathways of
%DIF < three group I intron ribozymes.
Neupane et al. \mbox{%DIFAUXCMD
\citep{neupane.nar11} }%DIFAUXCMD
applied single-molecule force spectroscopy
of the add adenine }%DIFDELCMD < \rb%%%
\DIFdel{,
to show how folding relates to
gene regulation;
see also \mbox{%DIFAUXCMD
\citep{woodside.cocb08,baird.jacs10,mitra.r11}}%DIFAUXCMD
.
%DIF < see \citep{Woodside.cocb08} for a recent comprehensive review of such methods.
%DIF < 
On the computational side, Morgan and Higgs \mbox{%DIFAUXCMD
\citep{morganhiggsbarrier}
}%DIFAUXCMD
appear to have been among the first
to have considered the computational problem of determining optimal and
near-optimal }%DIFDELCMD < {\em %%%
\DIFdel{folding pathways}%DIFDELCMD < } %%%
\DIFdel{between two metastable secondary
structures}%DIFDELCMD < \strAB %%%
\DIFdel{of a given RNA sequence. In \mbox{%DIFAUXCMD
\citep{flammphd}}%DIFAUXCMD
, Flamm
developed an event-driven Monte Carlo simulation, }%DIFDELCMD < \kinfold%%%
\DIFdel{,
...
Using }%DIFDELCMD < \rnasub %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep{wuchty.b99}}%DIFAUXCMD
, Flamm et al.
\mbox{%DIFAUXCMD
\citep{flamm} }%DIFAUXCMD
designed the exponential
time exact algorithm, }%DIFDELCMD < \barriers%%%
\DIFdel{, that computes the optimal folding
pathway between metastable structures }%DIFDELCMD < \strA %%%
\DIFdel{and
}%DIFDELCMD < \strB%%%
\DIFdel{. Since }%DIFDELCMD < \barriers
%DIFDELCMD < %%%
\DIFdel{may not converge, due to its reliance on the enumeration of possibly
exponentially many structures, in \mbox{%DIFAUXCMD
\citep{dotu.nar10} }%DIFAUXCMD
we developed a
local search (tabu) method that rapidly returns near-optimal folding pathways ; in benchmarking results \mbox{%DIFAUXCMD
\citep{dotu.nar10}}%DIFAUXCMD
, local search was shown
to outperform various methods, including direct and indirect path methods
of Morgan and Higgs \mbox{%DIFAUXCMD
\citep{morganhiggsbarrier}}%DIFAUXCMD
, breadth-first search with
bounded lookahead, }%DIFDELCMD < \ms{Findpath} %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep{flamm.r01}}%DIFAUXCMD
, and the
exponential time, exact method }%DIFDELCMD < \barriers %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep{flamm}}%DIFAUXCMD
, in producing
...
}\DIFdelend \DIFaddbegin \On{2} \DIFadd{space---efficient enough for the analysis of folding pathways of large
RNA sequences not tractable using }\rnatwofold\DIFadd{.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD < & \bfZ{x-\alpha_0,y-\beta_0}{i,j-1}\enspace + \\
%DIFDELCMD < & \sum_{\substack{s_k,s_j \in \bpSet, \\ i \le k<j}}
%DIFDELCMD < \left(
%DIFDELCMD < \boltzNuss{k,j}\quad
%DIFDELCMD < \sum_{\mathclap{u+u'=x-\alpha(k)}}\hspace{3.75em}
%DIFDELCMD < \sum_{\mathclap{v+v'=y-\beta(k)}}\quad
%DIFDELCMD < \bfZ{u,v}{i,k-1} \cdot \bfZ{u',v'}{k+1,j-1}
%DIFDELCMD < \right)
%DIFDELCMD < \end{split}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{split}
...
(see \DIFdelbegin %DIFDELCMD < \eqnref{fftbor:validBP}%%%
\DIFdelend \DIFaddbegin \DIFadd{equation \ref{eq:fftbor:validBP}}\DIFaddend ), and otherwise $E_0(k,j)=0$, and
$\alpha(k) =
...
guarantees that we can determine the coefficients \DIFdelbegin \DIFdel{$c_{rn+s}$ }\DIFdelend \DIFaddbegin \DIFadd{$z_{rn+s}$ }\DIFaddend of \fullZx,
...
\begin{theorem}
\DIFaddbegin \label{thm:ffttwo:recursions}
\DIFaddend Let $s_1,\dots,s_n$ be a given RNA sequence.
...
\DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD < &\emZof{i,j-1}{x} \cdot x^{\alpha_0n+\beta_0} + \\
%DIFDELCMD < &\sum_{\substack{s_k,s_j \in \bpSet,\\i\le k<j}}
%DIFDELCMD < \left(\boltzNuss{k,j}\cdot
%DIFDELCMD < \emZof{i,k-1}{x} \cdot \emZof{k+1,j-1}{x}\cdot x^{\alpha(k)n+\beta(k)} \right)
%DIFDELCMD < \end{split}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{split}
...
%DIF <  The proof is given in supplementary information.
\DIFaddbegin \DIFadd{The proof is given in }\Secref{sec:ffttwo:recursions}\DIFadd{.
...
for \DIFdelbegin \DIFdel{base pair distance}\DIFdelend \DIFaddbegin \bpd\DIFaddend , probably folklore.
...
and \DIFdelbegin \DIFdel{minimum free energy structure}\DIFdelend \DIFaddbegin \mfes\DIFaddend , $\text{MFE\,}^{r,s}_{1,n}$, over all
structures having \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend $r$ to \strA and \str to \strB.
\DIFdelbegin \DIFdel{The following lemma is not difficult to establish.
}\DIFdelend \end{lemma}
...
If the \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend between reference
...
\begin{align}
\nu^k = -(\nu\DIFdelbegin \DIFdel{^{-(M/2-k)}}\DIFdelend \DIFaddbegin \DIFadd{^{-(M_0-k)}}\DIFaddend ) = -\DIFdelbegin %DIFDELCMD < \overline{\nu^{M/2-k}}%%%
\DIFdelend \DIFaddbegin \overline{\nu^{M_0-k}}\DIFaddend .
\end{align}
\end{lemma}

Lemma \ref{lem:ffttwo:lemma1} is proved by \DIFdelbegin \DIFdel{simple }\DIFdelend induction;
Lemma \ref{lem:ffttwo:lemma2}
is proved \DIFaddbegin \DIFadd{in }\Secref{lem:fftbor:compconj} \DIFaddend by a computation involving
binomial coefficients;
Lemma \ref{lem:ffttwo:lemma3} is \DIFdelbegin \DIFdel{immediate
}\DIFdelend \DIFaddbegin \DIFadd{proved in }\Secref{sec:ffttwo:lemma3proof}
\DIFaddend by the parity observation above, resulting from Lemma \ref{lem:ffttwo:lemma1};
Lemma \ref{lem:ffttwo:lemma4} is \DIFdelbegin \DIFdel{elementary}\DIFdelend \DIFaddbegin \DIFadd{proved in }\Secref{sec:ffttwo:lemma4proof}\DIFaddend ,
relying on Euler's formula and trigonometric addition formulas.
\DIFdelbegin \DIFdel{Details proofs of
...
\DIFdelbegin \DIFdel{$\fullZx = \sum_{k=0}^{M/2-1} z_{2k+1} x^{2k+1}$}\DIFdelend \DIFaddbegin \DIFadd{$\fullZx = \sum_{k=0}^{M_0-1} z_{2k+1} x^{2k+1}$}\DIFaddend . But then
...
\DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend \dBP{\strA}{\strB} between \strAB is
even, then
...
\end{algorithmic}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Pseudocode for improved \ffttwo]{\DIFaddendFL 
Pseudocode to compute the $m$ most significant digits
for probabilities
...
having \DIFdelbegin \DIFdel{minimum free energy }\DIFdelend \DIFaddbegin \mfe \DIFaddend over all structures, whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend from
\strA was $k_0$.
...
no dangles, suppression of \DIFdelbegin \DIFdel{minimum free energy structure }\DIFdelend \DIFaddbegin \mfes \DIFaddend computations
...
\DIFdelbegin \DIFdel{$-9.660419$ }\DIFdelend \DIFaddbegin \DIFadd{$-9.6604$ }\DIFaddend kcal/mol, while \ffttwo obtains
\DIFdelbegin \DIFdel{$-9.660543$ }\DIFdelend \DIFaddbegin \DIFadd{$-9.6605$ }\DIFaddend kcal/mol; similarly, for attenuator RNA of length 73 nt,
\rnatwofold \ms{-d0} obtains a value of
\DIFdelbegin \DIFdel{$-22.171785$ }\DIFdelend \DIFaddbegin \DIFadd{$-22.1718$ }\DIFaddend kcal/mol, while \ffttwo obtains
\DIFdelbegin \DIFdel{$-22.173213$ }\DIFdelend \DIFaddbegin \DIFadd{$-22.1732$ }\DIFaddend kcal/mol.
...
with \rnatwofold, we restricted \DIFdelbegin \DIFdel{therange }\DIFdelend \DIFaddbegin \DIFadd{the range }\DIFaddend of $x,y$ in the same manner as done in the source code of \rnatwofold.
...
\end{subfigure}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[{\em (Top)}
Standard deviation of run times of \rnatwofold and \ffttwo
as a function of sequence length $n$.
{\em (Bottom)}
Minimum and maximum run times for \rnatwofold and \ffttwo]{\DIFaddendFL 
{\em (Top)}
Standard deviation of run times of \rnatwofold and \ffttwo
as a function of sequence length $n$.
{\em (Bottom)}
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[2D projection of energy landscape for Spliced Leader (SL) RNA
...
represents \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL between metastable structure \strA [resp.
\strBresp],
...
is computed from the values of \bfZ{x,y}{} output by \rnatwofold,
while in the \DIFdelbeginFL \DIFdelFL{right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{left }\DIFaddendFL panel, ensemble free energy is computed from
the values $\bfZ{x,y}{}=p(x,y) \cdot \fullZ$, where values $p(x,y)$ are output
by \rnatwofold.
...
compute the structures $M_{x,y}$ having \DIFdelbegin \DIFdel{minimum free energy }\DIFdelend \DIFaddbegin \mfe \DIFaddend over all
...
initially empty secondary structure to the target \DIFdelbegin \DIFdel{minimum free energy
}\DIFdelend \DIFaddbegin \mfe
\DIFaddend (MFE) structure. Examples of methods to determine optimal or
...
near-optimal folding pathways, \DIFaddbegin \DIFadd{the Basin Hopping Graph (BHG, \mbox{%DIFAUXCMD
\citep{kucharik:2014ft}}%DIFAUXCMD
) which uses a greedy algorithm to determine local minima
and approximate their barrier energies, }\DIFaddend etc. The program \barriers
...
\DIFaddbegin \ms{KFOLD} \DIFadd{\mbox{%DIFAUXCMD
\citep{dykeman:2015bxa} }%DIFAUXCMD
also implements Gillespie's algorithm,
but leverages memoization of transition rates to operate much more efficiently
than }\kinfold\DIFadd{. Meanwhile }\DIFaddend \ms{Kinefold} \citep{xayaphoummine.nar05} uses
...
secondary structures having \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend $x$ [resp. $y$] from
reference structures \strA [resp. \strBresp] of a given RNA sequence.
\DIFdelbegin \DIFdel{Mean first passage time }\DIFdelend \DIFaddbegin \Mfpt \DIFaddend (MFPT) is computed exactly by matrix inversion,
...
the $(i,j)$\DIFdelbegin \DIFdel{-th }\DIFdelend \DIFaddbegin \DIFadd{th }\DIFaddend entry of the $t$\DIFdelbegin \DIFdel{-th }\DIFdelend \DIFaddbegin \DIFadd{th }\DIFaddend power $P^t$ of matrix $P$  equals
$p_{i,j}^{(t)}$.
...
\DIFdelbegin \DIFdel{minimum free energy}\DIFdelend \DIFaddbegin \mfe\DIFaddend . These results justify the use of the Monte Carlo
...
\end{algorithmic}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Pseudocode for the Metropolis Monte Carlo algorithm]{\DIFaddendFL The function {\sc Metropolis} implements a discrete time simulation
...
$c_k = \left(p(0) \cdot T^{-1})\right)_k$, where the $j$\DIFdelbegin \DIFdel{-th }\DIFdelend \DIFaddbegin \DIFadd{th }\DIFaddend row of $T$
is the $j$\DIFdelbegin \DIFdel{-th }\DIFdelend \DIFaddbegin \DIFadd{th }\DIFaddend left eigenvector of $R$,
...
to be the \DIFdelbegin \DIFdel{minimum free energy structure}\DIFdelend \DIFaddbegin \mfes\DIFaddend , though this is not necessary for
...
\end{algorithmic}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Pseudocode for the Gillespie algorithm]{\DIFaddendFL The function {\sc Gillespie} implements a continuous time simulation of
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Overall organization of the \hermes software suite]{\DIFaddendFL Overall
organization of
...
structure, into a target structure, such as the \DIFdelbeginFL \DIFdelFL{minimum free energy
}\DIFdelendFL \DIFaddbeginFL \mfe
\DIFaddendFL (MFE) structure or possibly the Rfam \citep{Gardner.nar11} consensus
...
\seq, whose \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend with $x$ is one. Define the Markov
...
The exact value of \DIFdelbegin \DIFdel{mean first passage time }\DIFdelend \DIFaddbegin \mfpt \DIFaddend (MFPT) can be computed as
...
exactly equal to the \DIFdelbegin \DIFdel{mean first passage time }\DIFdelend \DIFaddbegin \mfpt \DIFaddend from the structure with
...
\DIFdelbegin \DIFdel{mean first passage time }\DIFdelend \DIFaddbegin \mfpt \DIFaddend is mathematically exact, we consider that MFPT
...
\DIFdelbegin %DIFDELCMD < \eqnref{hermes:mfptXitionProb}%%%
\DIFdelend \DIFaddbegin \DIFadd{equation \ref{eq:hermes:mfptXitionProb}}\DIFaddend ).
...
\ffttwo. Here, $d_0$ is the \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend between structures
...
target structure \strB, we proceed in the following fashion. Let \dBP{\strA}{\strB} denote the \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend between structures
...
Let $d_0= \dBP{\strA}{\strB}$, the \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend between initial structure
...
landscape.
\DIFdelbegin \DIFdel{Vertices }\DIFdelend \DIFaddbegin \DIFadd{Indices $(x,y)$ }\DIFaddend in the rate matrix \DIFdelbegin \DIFdel{represent a subset of
structures compatible with the input sequence as modeled by
}%DIFDELCMD < \ffttwo %%%
\DIFdel{(in a fashion similar to }%DIFDELCMD < \fftmfpt%%%
\DIFdel{,
explained in
}%DIFDELCMD < \Secref{subsec:hermes:fftmfpt}%%%
\DIFdel{), }\DIFdelend \DIFaddbegin \DIFadd{correspond to all strucutres having
}\bpd \DIFadd{$x$ }[\DIFadd{resp. $y$}] \DIFadd{to input structure }\strA [\DIFadd{resp. }\strBresp]\DIFadd{,
}\DIFaddend which makes the graph size more tractable than structural
...
as \DIFdelbeginFL \DIFdelFL{$0.17893806$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.1789$ }\DIFaddendFL should equal the Boltzmann probability \DIFdelbeginFL \DIFdelFL{$0.17909227$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.1791$}\DIFaddendFL ,
...
other low energy structures at the same \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL $x_1$
[resp. $y_1$] from \strA [resp. \DIFdelbeginFL %DIFDELCMD < \strB%%%
\DIFdelendFL \DIFaddbeginFL \strBresp\DIFaddendFL ] as that of the alternate suboptimal
structure, then we expect that the occupancy probability \DIFdelbeginFL \DIFdelFL{$0.03003426$
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0300$
}\DIFaddendFL for the $(x_1,y_1)$ be approximately the Boltzmann probability
\DIFdelbeginFL \DIFdelFL{$0.03005854$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0301$ }\DIFaddendFL of the alternate structure.}
...
compute the \eqt $t$ (from \DIFdelbegin %DIFDELCMD < \eqnref{hermes:solutionEquilibriumTime}%%%
\DIFdelend \DIFaddbegin \DIFadd{equation \ref{eq:hermes:solutionEquilibriumTime}}\DIFaddend ),
...
such that for \DIFdelbegin \DIFdel{$t \in \{t_0+\tD{},t_0+\tD{2},t_0+\tD{3},t_0+\tD{4}\}$}\DIFdelend \DIFaddbegin \DIFadd{$t \in \{t_0+\tD,t_0+2\tD,t_0+3\tD,\dots,t_0+(w-1)\cdot\tD\}$}\DIFaddend , the absolute difference
$|p(t)[\xEnd] - p(t_0)[\xEnd]| < \epsilon$, for $\epsilon =
10^{-4}$ and step size \DIFdelbegin %DIFDELCMD < \tD{}%%%
\DIFdelend \DIFaddbegin \tD \DIFadd{across a window of length $w$}\DIFaddend . In systems where there are local energy minima, this approach has the
...
\DIFdelbegin \DIFdel{$w = 5, \tD{} = 10^{-3}, \delta = 10^{-3}$ }\DIFdelend \DIFaddbegin \DIFadd{$w = 5, \tD = 10^{-3}, \delta = 10^{-3}$ }\DIFaddend and $\epsilon = 10^{-4}$, we compute the estimated
...
\DIFdelbeginFL \DIFdelFL{$w = 5, \tD{} = 10^{-3}, \delta = 10^{-3}, \epsilon = 10^{-4}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$w = 5, \tD = 10^{-3}, \delta = 10^{-3}, \epsilon = 10^{-4}$ }\DIFaddendFL \rule[-1.5ex]{0pt}{0pt} \\
{\sc Output:} & Time $\log_{10}(t)$ at which $p_{\xEnd}(10^t)$ varies by no
more than $\epsilon$ within \DIFaddbeginFL \DIFaddFL{a }\\ & \DIFaddFL{window of size }\DIFaddendFL $w$\DIFaddbeginFL \DIFaddFL{, having step size }\tD
\DIFaddendFL \rule[-1.75em]{0pt}{0pt} \\
\hline \rule[0ex]{0pt}{0pt}
\end{tabular*}
\begin{algorithmic}[1]
\DIFdelbeginFL %DIFDELCMD < \Function{EstimateEquilibrium}{\xStart, \xEnd, \tMin, \tMax, \tD{}, $\delta$, $\epsilon$, $w$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \Function{EstimateEquilibrium}{\xStart, \xEnd, \tMin, \tMax, \tD, $\delta$, $\epsilon$, $w$}
\DIFaddendFL \State \DIFdelbeginFL \DIFdelFL{$t \gets \Call{SoftBound}{\xEnd, \tMin, \tMax, \tD{}, \delta, s}$
}%DIFDELCMD < \If{$\Call{WindowEq}{\xEnd, t, \tD{}, w, \epsilon}$}
%DIFDELCMD < \While{$t > \tMin$ AND $\Call{WindowEq}{\xEnd, t, \tD{}, w, \epsilon}$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t \gets \Call{SoftBound}{\xEnd, \tMin, \tMax, \tD, \delta, s}$
}\If{$\Call{WindowEq}{\xEnd, t, \tD, w, \epsilon}$}
\While{$t > \tMin$ AND $\Call{WindowEq}{\xEnd, t, \tD, w, \epsilon}$}
\DIFaddendFL \State \DIFdelbeginFL \DIFdelFL{$t \gets t - \tD{}$
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t \gets t - \tD$
}\DIFaddendFL \EndWhile
\EndIf
\DIFdelbeginFL %DIFDELCMD < \While{$t < \tMax$ AND NOT $\Call{WindowEq}{\xEnd, t, \tD{}, w, \epsilon}$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \While{$t < \tMax$ AND NOT $\Call{WindowEq}{\xEnd, t, \tD, w, \epsilon}$}
\DIFaddendFL \State \DIFdelbeginFL \DIFdelFL{$t \gets t + \tD{}$
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t \gets t + \tD$
}\DIFaddendFL \EndWhile
\If{$t = \tMin$}
...
\begin{algorithmic}[1]
\algrestore{eqEst}
\DIFdelbeginFL %DIFDELCMD < \Function{SoftBound}{$i$, \tMin, \tMax, \tD{}, $\delta$, $s$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \Function{SoftBound}{$i$, \tMin, \tMax, \tD, $\delta$, $s$}
\DIFaddendFL \State $i_{\infty} \gets p_i(10^{\tMax})$
\Comment{From equation \ref{eq:hermes:solutionMasterEquationForSingleStateFixedStartPop}}
\State $t \gets \tMin + \frac{\tMax - \tMin}{2}$
\State $t' \gets \tMax$
\DIFdelbeginFL %DIFDELCMD < \While{$|t - t'| > \tD{}$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \While{$|t - t'| > \tD$}
\DIFaddendFL \Comment{Binary search for time $t$ within $\delta$ of \tMax}
...
\State \textbf{return} $t$
\EndFunction
\DIFdelbeginFL %DIFDELCMD < \Function{WindowEq}{$x$, $t$, \tD{}, $w$, $\epsilon$}
%DIFDELCMD < \Comment{Is $x$ within $\epsilon$ of equilibrium for $[t, \tD{(w-1)}]$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \Function{WindowEq}{$x$, $t$, \tD, $w$, $\epsilon$}
\Comment{Is $x$ within $\epsilon$ over $[t, t+(w-1)\cdot\tD]$}
\DIFaddendFL \For{$i \gets 1, w - 1$}
\DIFdelbeginFL %DIFDELCMD < \If{$|p_x(10^{t+\tD{i}}) - p_x(10^t)| > \epsilon$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \If{$|p_x(10^{t+i\cdot\tD}) - p_x(10^t)| > \epsilon$}
\DIFaddendFL \State \textbf{return} \textbf{false}
\EndIf
\EndFor
...
\end{algorithmic}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Pseudocode for estimating equilibrium time]{\DIFaddendFL The function {\sc EstimateEquilibrium} computes the smallest $t_0 > t'$,
such that for \DIFdelbeginFL \DIFdelFL{$t \in \{t_0+\tD{},t_0+\tD{2},t_0+\tD{3},t_0+\tD{4}\}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t \in \{t_0+\tD,t_0+2\tD,t_0+3\tD,\dots,t_0+(w-1)\cdot\tD\}$}\DIFaddendFL ,
...
In using the approach outlined in \Figref{hermes:eqEst}, the \DIFdelbegin \DIFdel{correlation
}\DIFdelend \DIFaddbegin \DIFadd{root-mean-square
deviation (RMSD)
}\DIFaddend between $p_{\xEnd}(t_0)$ and $\frac{\boltzf{\xEnd}}{\fullZ}$ is \DIFdelbegin \DIFdel{$0.9997$, and
visual }\DIFdelend \DIFaddbegin \DIFadd{$0.0041$, where
RMSD is defined as
}

\begin{align}
\DIFadd{\label{eq:hermes:rmsd}
\sqrt{\frac{\sum_{i=1}^{n} (\widehat{x_i} - x_i)^2}{n}}.
}\end{align}

\DIFadd{Visual }\DIFaddend inspection of the estimated equilibrium times
(\Figref{hermes:eqEstFromRnaEq}) are much better than the
na\"{\i}ve approach used by \treekin\DIFaddbegin \DIFadd{, which uses a simple sliding window for
all $x \in Q$}\DIFaddend .
We also allow defining the \eqt to be the
smallest $t_0$, such that for
\DIFdelbegin \DIFdel{$t \in \{t_0+\tD{},t_0+\tD{2},t_0+\tD{3},t_0+\tD{4}\}$}\DIFdelend \DIFaddbegin \DIFadd{$t \in \{t_0+\tD,t_0+2\tD,t_0+3\tD,\dots,t_0+(w-1)\cdot\tD\}$}\DIFaddend , the
...
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Example of the differences in using a sliding window approach for estimating \eqt versus a more robust approach, outlined in Algorithm \ref{fig:hermes:eqEst}]{\DIFaddendFL Example of the differences in using the simple sliding window
...
probability---the \DIFdelbeginFL \DIFdelFL{correlation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{root-mean-square deviation (RMSD, equation
\ref{eq:hermes:rmsd}) }\DIFaddendFL across the dataset of 1,000 sequences is \DIFdelbeginFL \DIFdelFL{$0.9997$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0041$
}[\DIFaddFL{resp}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{$0.0491$}] \DIFaddFL{for the approach described in Algorithm \ref{fig:hermes:eqEst}
}[\DIFaddFL{resp. simple sliding window}]\DIFaddFL{.}\DIFaddendFL }
...
structures, such that the \DIFdelbegin \DIFdel{minimum free energy }\DIFdelend \DIFaddbegin \mfe \DIFaddend is less than or equal to
$-5.5$ kcal/mol.

For example, one of the 1,000 sequences is \ms{ACGCGACGUGCACCGCACGU} with
\DIFdelbegin \DIFdel{minimum free energy structure }\DIFdelend \DIFaddbegin \mfes \DIFaddend \ms{.....((((((...))))))} having free
...
same figure depicts the \DIFdelbegin \DIFdel{minimum free energy structure }\DIFdelend \DIFaddbegin \mfes \DIFaddend of the
...
Viroid (PLMVd) AJ005312.1/282-335]{\DIFaddendFL 
...
model were used, since the \DIFdelbeginFL \DIFdelFL{minimum free energy structure }\DIFdelendFL \DIFaddbeginFL \mfes \DIFaddendFL determined by
...
\ms{CCGAUUGGCGAAAGGCCACC}]{\DIFaddendFL {\em (Left)} Histogram of \kinfold folding times for 20-mer
...
versus \DIFdelbeginFL %DIFDELCMD < \rnaeq %%%
\DIFdelendFL \DIFaddbeginFL \msresp{RNAeq} \DIFaddendFL {\em (Left)} and for \kinfold versus
\DIFdelbeginFL %DIFDELCMD < \rnaeq %%%
\DIFdelendFL \DIFaddbeginFL \msresp{RNAeq} \DIFaddendFL {\em (Right)}. }
...
\caption{Scatter plots of the natural logarithm of times from \rnamfpt
versus \DIFdelbeginFL %DIFDELCMD < \kinfold %%%
\DIFdelendFL \DIFaddbeginFL \msresp{Kinfold} \DIFaddendFL {\em (Left)} and for \rnamfpt versus \DIFdelbeginFL %DIFDELCMD < \fftmfpt %%%
\DIFdelendFL \DIFaddbeginFL \msresp{FFTmfpt} \DIFaddendFL {\em (Right)}. }
...
versus \DIFdelbeginFL %DIFDELCMD < \fftmfpt %%%
\DIFdelendFL \DIFaddbeginFL \msresp{FFTmfpt} \DIFaddendFL {\em (Left)} and for \fftmfpt versus \DIFdelbeginFL %DIFDELCMD < \ffteq
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \msresp{FFTeq}
\DIFaddendFL {\em (Right)}. }
...
\begin{tabularx}{\linewidth}{c *{8}{L}}
 ...
 \end{tabularx}
 \DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Table of Pearson correlation coefficients for various methods to compute or approximate RNA secondary structure folding kinetics]{\DIFaddendFL Table of Pearson correlation coefficients for various methods to compute or approximate RNA secondary structure folding kinetics. Lower [resp. upper] triangular entries are with [resp. without] the Hastings correction for Markov chain probability matrices. The methods are: \rnamfpt (\mfpt, computed by matrix inversion for the Markov chain consisting of all secondary structures, with move allowed between structures differing by one base pair), \rnaeq (\eqt, computed by spectral decomposition of a rate matrix comprising all secondary structures to compute population fraction $P(t)$ at time $t$), \kinfold (an implementation of Gillespie's Algorithm to approximate refolding pathways using an event-based Monte Carlo simulation), \fftmfpt (\mfpt for Markov chain consisting of ``grid point'' states $(x,y)$ with probability $P(x,y)=\sum_S exp(-E(S)/RT)/Z$, computed by \ffttwo, where the sum is taken over structures having \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL $x$ to the empty structure and $y$ to the MFE structure, \rnatwofold (\mfpt, computed as previously explained, but using \rnatwofold in place of \ffttwo to compute $P(x,y)$), \fftbor (\mfpt, computed for the Markov chain consisting of states $0,1,\dots,n$, for which $P(x) = \sum_S \exp(-E(S)/RT)/Z$, where the sum is taken over all secondary structures whose \DIFdelbeginFL \DIFdelFL{base pair distance }\DIFdelendFL \DIFaddbeginFL \bpd \DIFaddendFL is $x$ from the MFE structure), \ms{BarriersEq} (\eqt, computed using spectral decomposition on the Markov process consisting of ``grid point'' states output from \ms{Barriers}), and \ffteq (\eqt, computed in the same fashion as \ms{BarriersEq} using a Markov process derived from the energy landscape output by \ffttwo). }
...
\citep{infernal}. For this reason, databases such as \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend \citep{nawrocki:2014uy}
...
those sequences having \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend $k$ from an arbitrary input structure
...
by these
programs we were able to classify \DIFdelbegin \DIFdel{RFam }\DIFdelend \DIFaddbegin \DIFadd{Rfam }\DIFaddend families using five-fold cross validation
...
$(x,y)$ on the discrete energy landscape corresponds to the Boltzmann
probability for those structures \str which have \DIFdelbegin \DIFdel{base pair distance }\DIFdelend \DIFaddbegin \bpd \DIFaddend $x$ [resp. $y$]
...
%DIF <  \appendix % Cue to tell LaTeX that the following 'chapters' are Appendices
\DIFaddbegin \appendix %DIF >  Cue to tell LaTeX that the following 'chapters' are Appendices
\DIFaddend 
...
\chapter{\DIFadd{FFTbor Appendix}}
\label{ch:fftborapp}

\lhead{FFTbor Appendix}

\section{\DIFadd{Full recursions for }\texorpdfstring{\emZ{i,j}}{}
\DIFadd{for the Turner energy model}}
\label{sec:fftbor:turner}

\DIFadd{To compute $\emZ{} = \emZ{1,n}$ given input structure }\str\DIFadd{, we use the recursions
}

\begin{align}
\DIFadd{\label{eq:fftborapp:zx}
...
\DIFadd{where $d_0 = 1$ if $j$ is base paired
...
\DIFadd{We compute }\emZB{} \DIFadd{using the recursion
}

\begin{align}
\DIFadd{\label{eq:fftborapp:zbx}
\begin{split}
...
\DIFadd{where $d_2 = \dBP{\str_{[i,j]}}{\{(i,j)\}}$,
...
\DIFadd{The final recursion, for computing }\emZM{}\DIFadd{, is
}

\begin{align}
\DIFadd{\label{eq:fftborapp:zmx}
\begin{split}
...
\DIFadd{where $d_5 = \dBP{\str_{[i,j]}}{\str_{[k,j]}}$ and $d_6
...
region (the }\emZB{} \DIFadd{term) or
more than one (the }\emZM{}\DIFadd{--}\emZB{} \DIFadd{term).
...
\chapter{\DIFadd{FFTbor2D Appendix}}
\label{ch:ffttwoapp}
...
\DIFadd{To compute $\emZ{} = \emZ{1,n}$ given input structures }\strAB\DIFadd{,
we use the recursions
}

\begin{align}
\DIFadd{\label{eq:ffttwoapp:zx}
...
\section{\DIFadd{Proof for Theorem \ref{thm:ffttwo:recursions}}}
\label{sec:ffttwo:recursions}

\begin{proof}
\DIFadd{Recall that if $F$ is an arbitrary
polynomial }[\DIFadd{resp. analytic}] \DIFadd{function, then $[x^{rn+s}]F(x)$
denotes the coefficient of monomial $x^{rn+s}$ in the
Taylor expansion of $F(x)$. For instance, in
}\eqnref{ffttwo:pOfX}\DIFadd{, $[x^{rn+s}]p(x) = p_{rn+s}$, and in
}\eqnref{ffttwo:zOfX}\DIFadd{, $[x^{rn+s}]\fullZx = z_{rn+s}$.
}

\DIFadd{By definition, it is clear that $\emZ{i,j}=1$ if $i \leq j \leq i + \theta$,
where we recall that $\theta = 3$ is the minimum number of unpaired bases in
a hairpin loop. For $j > i + \theta$, we have
}
...
\section{\DIFadd{Proof for Lemma \ref{lem:ffttwo:lemma3}}}
\label{sec:ffttwo:lemma3proof}

\begin{proof}
\DIFadd{The lemma states that if the }\bpd \DIFadd{$d_0$ between reference
structures }\strAB \DIFadd{is even, then $\emZof{}{-\alpha}=\emZof{}{\alpha}$, while if
the distance is odd, then $\emZof{}{-\alpha}=-\emZof{}{\alpha}$.
...
\section{\DIFadd{Proof for Lemma \ref{lem:ffttwo:lemma4}}}
\label{sec:ffttwo:lemma4proof}

\begin{proof}
\DIFadd{Recall Euler's formula in complex analysis:
...
\DIFadd{This completes the proof of the lemma.
}\end{proof}

\DIFaddend % \addtocontents{toc}{\vspace{2em}} % Add a gap in the Contents, for aesthetics
